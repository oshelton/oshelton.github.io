...
class Build : NukeBuild
{
    ...
    private static void GeneratePostsFile(AbsolutePath blogDataDir, IReadOnlyList<PostMetadata> foundMetadata)
    {
		...
		tmpPostsDataFilePath.AppendAllLines(foundMetadata.Select(metadata =>
			$"PostMetadata['{metadata.Id}'] = {{ title: `{metadata.Title}`, id: '{metadata.Id}', posted: '{metadata.Posted}', lastModified: '{metadata.LastModified}', summary: `{metadata.Summary}`, tags: [ `{string.Join("`, `", metadata.Tags)}` ] }};"));
	...
	private static void GenerateSearchFile(AbsolutePath blogDataDir, IReadOnlyList<PostMetadata> foundMetadata)
	{
		...
		// Write out warning and tags preamble.
		...
		tmpSearchDataFilePath.AppendAllLines(allTags.Select(tag =>
		{
			var postIdsForTag = foundMetadata
				.Where(x => x.Tags.Contains(tag))
				.Select(x => $"'{x.Id}'")
				.Join(", ");
			return $"TagsToPostIds['{tag}'] = [{postIdsForTag}];";
		}));

		// Write out post ids to posted dates.
		tmpSearchDataFilePath.AppendAllLines(new[]
		{
			"",
			"/**",
		...
		tmpSearchDataFilePath.AppendAllLines(foundMetadata.Select(metadata => $"\t{{ id: '{metadata.Id}', posted: new Date({metadata.Posted.Year}, {metadata.Posted.Month - 1}, {metadata.Posted.Day}) }},"));
